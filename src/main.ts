    ...
    for (let j = 0; j < array.length - i - 1; j += 1) {
      if (array[j] > array[j + 1]) {
        ...
        for (let k = 0; k < bars.length; k += 1) {
          if (k !== j && k !== j + 1) {
            (bars[k] as HTMLDivElement).style.backgroundColor = "teal";
          }
        }
        let temp = array[j];
        array[j] = array[j + 1];
        ...
        (bars[j] as HTMLDivElement).style.backgroundColor = "aquamarine";
        (bars[j] as HTMLDivElement).style.height =
          (array[j] * factorHeight) / 16 + "rem";

        (bars[j + 1] as HTMLDivElement).style.backgroundColor = "aqua";
